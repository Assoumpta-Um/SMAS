...
const ConfirmationTable = () => {
  ...
  const deleteConfirmation = (e, id) => {
    e.preventDefault();
    axios
      .delete("http://localhost:8186/smas/confirmation/" + id)
      .then((res) => {
  ...
  const fetchData = () => {
    axios
      .get("http://localhost:8186/smas/confirmation/")
      .then((res) => {
        console.log(res);
        setBapt(res.data);
      })
  ...
  return (
    <div className="headers">
      <h3>
        <span>Paroisse Saint Vincent Pallotti Gikondo Confirmation Request.</span>
      </h3>
          ...
          {bapt.map((user) => (
            <tbody className="content" key={user.id}>
              <tr>
                ...
                <td>{user.confirmationDate}</td>
                <td>{user.confirmationNumber}</td>
                <td>
                  <BsFillTrashFill
                    onClick={(e, id) => deleteConfirmation(e, user.id)}
